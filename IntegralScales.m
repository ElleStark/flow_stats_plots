...
dataStack = h5read(h5_file,'/Flow Data/u',[1 1 startFrame],[size(xMesh,1) size(xMesh,2) endFrame]); % About 10 GB RAM per variable (for all time)
%dataStack = h5read(h5_file,'/Flow Data/u',[600 1 startFrame],[10 10 endFrame]);
...
total_nocross = 0;
for i=1:Ny
    [Lux(i), Tux(i), n_nocross] = Lx(data_vecs(i, :), data_mean(i), dt, 'method', 1);
    total_nocross = total_nocross + n_nocross;
    if i/1000==0
        pct = i / Ny * 100;
...
fprintf('number of no zero crossing functions: %. \n', total_nocross)
...
save('ignore/extended_sim/Tux_array_u_extendedsim.mat', 'Tux_array')
...
figure; 
% pcolor(xMesh(1:3, 1:3), yMesh(1:3, 1:3), Tux_array);
pcolor(Tux_array);
shading interp;
axis equal;
axis tight;
...
print(gcf, 'Plots/ITS_u_extendedsim.png', '-dpng', '-r600');
